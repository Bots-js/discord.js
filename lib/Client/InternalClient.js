...
var _nodeApollo = require("node-apollo");
...
var InternalClient = (function () {
				...
				return new Promise(function (resolve, reject) {
					...
					var check = function check(data) {
						if (data.t === "VOICE_SERVER_UPDATE") {
							...
							var chan = new _nodeApollo.Connection(_this4.apollo, {
								channelId: channel.id,
								sessionId: session,
								guildId: server.id,
								userId: _this4.client.user.id,
								endpoint: endpoint,
								token: token
							});
	...
	InternalClient.prototype.loginWithToken = function loginWithToken(token, email, password) {
		...
		return this.getGateway().then(function (url) {
			...
			if (!self.apollo) {
				self.apollo = new _nodeApollo.Apollo({ token: _this9.token });
			}
